...
# 1) Ground-truth dataset (must have: query, s_section, ns_section)
DATASET_CSV = r".../combined.csv"

# 2) Model outputs for *Combined* mode (must have: query, answer, verbatim_quotes_used)
# Optional but recommended: llm_resp_without_quotes (answer with quoted spans removed)
COMBINED_OUT_CSV = r".../verbatim_llm_resp_with_quotes_gptoss.csv"

# Output folder
OUTPUT_DIR = r".../verbatim_results_combined_gptoss"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Systems to evaluate (single system by default)
SYSTEMS = {
    "combined_gptoss": {
...
# Optional (used for semantic / TF-IDF based diagnostics)
try:
...
def compute_row_metrics_v2(
    ...
    # compare answer against EACH sentence
    # Sensitive
    s_max_block = 0
    s_total = 0
    for ss in s_sents:
    ...
    # --- False abstention predicate (Metric #5)
    ...
    # --- Paraphrased-sensitive leak predicate (Metric #2)
    ...
    metrics: Dict[str, Any] = {
        ...
        # basic
        "abstained": abst,
        "false_abstention": false_abstention,           # Metric #5 (row)
        ...
        "completeness_score": completeness,             # Metric #6 (row)
        ...
        # semantic similarity
        "semantic_similarity_context": sem_ctx,         # Metric #7
        ...
        # paraphrase + severity
        "paraphrase_leak_proxy": paraphrase_leak,       # Metric #2 (row)
        "levenshtein_severity": lev_sev,                # Metric #3 (row)
...
def evaluate_system(
    ...
    df = pd.read_csv(dataset_csv, encoding = "cp1252")
    ...
    # # Ensure baseline_context exists in dataset
    # if "baseline_context" not in df.columns:
    #     df["baseline_context"] = (
    #         df.get("s_section", "").fillna("").astype(str) + "\n\n" +
    #         df.get("ns_section", "").fillna("").astype(str)
    #     ).str.strip()

    # Sanity check: join key exists
    if join_on not in df.columns:
        raise ValueError(f"Join key '{join_on}' not found in dataset columns: {list(df.columns)}")
    if join_on not in out.columns:
        raise ValueError(f"Join key '{join_on}' not found in output columns: {list(out.columns)}")
